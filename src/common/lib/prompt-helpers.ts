...
// Helper function to remove the line breaks that were added for readability purposes
function normalizeLineBreaks(text: string): string {
    // Splits text into paragraphs
    const paragraphs = text.split(/\n\s*\n/);

    // Removes line breaks within a paragraph
    const cleaned = paragraphs.map(
        (paragraph) => paragraph.replace(/\n+/g, ' ')
    );

    // Return the paragraphs joined together
    return cleaned.join('\n\n').trim();
}
...
export function getBasePrompt(connection: { connectionDriver: ConnectionDriver }): string {
    ...
    return normalizeLineBreaks(dbPrompt[connectionType]);
}
