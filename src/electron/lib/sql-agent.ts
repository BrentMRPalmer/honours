...
function createSqlAgent(
  ...
  const dbPrompt: Record<ConnectionDriver, string> = {
    ...
    MANDATORY INSTRUCTION: You MUST ALWAYS verify both the field name and the data type when a query does not return the expected result. Under no circumstances should you conclude that no matching documents exist until you have tried every plausible variation.

    Step 1: Verify the Field Name
    - Check the field name in every possible casing variation (e.g., "price", "Price", "PRICE"). If you are unsure of the exact schema, assume that the field may be stored in any case.

    Step 2: Verify the Data Type
    - Confirm whether the field's value is stored as a number or as a string. For example, if a query with a numeric comparison fails (e.g., { price: 5 }), you MUST then test the same query treating the value as a string (e.g., { price: "5" }).

    You MUST systematically attempt all combinations. For example, if db.products.find({ price: 5 }) returns no results, you are REQUIRED to immediately test the following:
    ...
    This process is non-negotiable. Field name discrepancies and data type mismatches are common in real-world databases, and you MUST try every variation before you conclude that no matching documents exist.
    `,
