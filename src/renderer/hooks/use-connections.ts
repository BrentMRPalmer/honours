...
interface UseConnections {
  ...
  addConnection: (name: string, driver: ConnectionDriver, config: unknown) => Promise<void>;
  updateConnection: (id: string, name: string, driver: ConnectionDriver, config: unknown) => Promise<void>;
  deleteConnection: (id: string) => Promise<void>;
  testConnection: (driver: ConnectionDriver, config: unknown) => Promise<{ success: boolean; message: string }>;
}
...
const useConnections = create<UseConnections>()((set, get) => ({
  ...
  async addConnection(name: string, driver: ConnectionDriver, config: unknown) {
    try {
      const connection = await window.SettingsController.addConnection(name, driver, config);
      ...
      set(state => ({ 
        ...
        activeConnectionId: state.connections.length === 0 ? proxyConnection.id : state.activeConnectionId
      }));
    } catch (error) {
  ...
  async updateConnection(id: string, name: string, driver: ConnectionDriver, config: unknown) {
    try {
      const updatedConnection = await window.SettingsController.updateConnection(id, name, driver, config);
  ...
  async deleteConnection(id: string) {
    try {
      ...
      if (success) {
        ...
        const newActiveId = id === activeConnectionId
          ? (updatedConnections.length > 0 ? updatedConnections[0].id : '')
          : activeConnectionId;
