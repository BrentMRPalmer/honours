          ...
          >
            {tabManager.tabs.map(({ id, title }) => (
              <SortableItem key={id}>
                <TabsTrigger
                  value={id}
                  className='data-[state=inactive]:bg-accent relative flex h-6 w-full min-w-40 items-center justify-center gap-2 hover:bg-gray-200 data-[state=inactive]:hover:bg-gray-200'
                  onMouseDown={(event) => {
