            ...
            <ConnectionForm
              onSave={(data) => {
                if (editingConnection) {
                  ...
                  addConnection(data.name, data.driver, data.config);
                }
                setIsOpen(false);
              }}
              editingConnection={
                editingConnection
                  ? connections.find((c) => c.id === editingConnection)
                  : null
              }
            />
          </DialogContent>
        ...
        {connections.map((connection) => (
          ...
          >
            ...
            <div className='overflow-hidden text-xs text-gray-500'>
              {connection.driver &&
                renderConnectionDetails(connection.driver, connection.config)}
            </div>
          </div>
        ))}
      </div>
...
function ConnectionForm({ onSave, editingConnection }: ConnectionFormProps) {
  ...
  const [config, setConfig] = useState<any>(() => {
    ...
    return editingConnection?.config
      ? { ...editingConnection.config }
      : getDefaultConfig(editingConnection?.driver || 'sqlite');
  });
    ...
    <>
          ...
          <>
            ...
            <div className='grid grid-cols-4 items-center gap-4'>
              ...
              <Input
                ...
                value={config.port}
                onChange={(e) =>
                  updateConfigField('port', parseInt(e.target.value, 10))
                }
                className='col-span-3'
                type='number'
              />
      ...
      <DialogFooter className='flex justify-between gap-2'>
        <div className='flex-1'>
          {testStatus && (
            <div
              className={`flex items-center text-sm ${testStatus.success ? 'text-green-600' : 'text-red-600'}`}
            >
              {testStatus.success ? (
